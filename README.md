...
- `pi-continue-after-compaction` - standalone local checkout at `~/pi-continue-after-compaction`; watches auto-threshold compactions only and sends an extension-originated `continue` if no next turn starts. Manual `/compact` and `/pi-vcc` are ignored.
